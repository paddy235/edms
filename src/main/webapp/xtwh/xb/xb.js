Ext.onReady(function() {
    ...
    //alert("aaa");
 	...
 	//渲染返回html代码
 	 ...
 	 {
 	     return '<a  href="xb_del.jsp?id='+value+'">删除</a>';
 	 };
    ...
    var columns = new Ext.grid.ColumnModel([
        ...
        {header:'线别代码',dataIndex:'XBID',width:60},
        {header:'线别名称',dataIndex:'XBMC',width:60,sortable:true},
        {header:'删除',dataIndex:'XBID',width:60,renderer:renderDel,fixed:true}
    ]);
   
    
    var planRecord = Ext.data.Record.create([
    	{name:'XBID',type:'string'},
    	{name:'XBMC',type:'string'}
    	]);
    ...
    var store = new Ext.data.Store({
        //proxy告诉我们从哪里获得数据
        proxy: new Ext.data.HttpProxy({url:'xb_json.jsp'}),        
        //reader告诉我们如何解析这个数据
   ...
   //Ext.form.Field.prototype.msgTarget = 'side';// 设置控件的错误信息显示位置，主要可选的位置有：qtip,title,under,side,[elementId]
    ...
    simple_Grid.addListener('rowclick', function(simple_Grid,rowIndex,event){
    	...
    	//alert(rowIndex);
    	//alert(record.get('ID')+' '+record.get('CODE')+' '+record.get('NAME'));
