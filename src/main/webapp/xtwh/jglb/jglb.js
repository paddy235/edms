Ext.onReady(function() {
    ...
    //alert("aaa");
 	...
 	//渲染返回html代码
 	 ...
 	 {
 	     return '<a  href="jglb_del.jsp?JGLBDM='+value+'">删除</a>';
 	 };
    ...
    var columns = new Ext.grid.ColumnModel([
        ...
        {header:'机构类别代码',dataIndex:'JGLBDM',width:60},
        {header:'机构类别名称',dataIndex:'JGLBMC',width:60,sortable:true},
        {header:'删除',dataIndex:'JGLBDM',width:60,renderer:renderDel,fixed:true}
    ]);
    ...
    var store = new Ext.data.Store({
        //proxy告诉我们从哪里获得数据
        proxy: new Ext.data.HttpProxy({url:'jglb_json.jsp'}),        
        //reader告诉我们如何解析这个数据
    ...
    var my_jg="";
    	...
    	my_jg=Ext.getCmp("JGLBDM").getValue();    	
    	Ext.getCmp("JGLBDM").disable();
    });	
            ...
            handler : function() {
            	if (!simpleForm_Save.form.isValid()) {return };
           		if (simpleForm_Save.form.isValid()) {
                	simpleForm_Save.form.doAction('submit', {
                                               waitMsg:'修改中,请稍侯...',  
                                               url : 'Update.jsp?JGLBDM='+my_jg,
                                               method : 'post',
                                               params : '',
                                               success : function(form, action) {
                                               		simpleForm_Save.buttons[0].setVisible(true);
                                               		simple_Grid.getStore().reload();
                                               		Ext.getCmp("JGLBDM").enable();
                                               		Ext.Msg.alert('消息',action.result.msg);
                                               },
                                               failure : function() {
