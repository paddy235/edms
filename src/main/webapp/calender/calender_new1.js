...
function setdayoff(tt,obj) 
{
  ...
  DCellWeb1.style.top = 225;
  event.returnValue=false;
}
function setdayXY(tt,obj,x,y) 
{
  if (arguments.length >  4){alert("对不起！传入本控件的参数太多！");return;}
  if (arguments.length == 0){alert("对不起！您没有传回本控件任何参数！");return;}
  var dads  = document.all.meizzDateLayer.style;var th = tt;
  var ttop  = tt.offsetTop+y;     
  var thei  = tt.clientHeight;  
  var tleft = tt.offsetLeft+x;    
  var ttyp  = tt.type;          
  while (tt = tt.offsetParent){ttop+=tt.offsetTop; tleft+=tt.offsetLeft;}
  dads.top  = (ttyp=="image")? ttop+thei : ttop+thei+6;
  dads.left = tleft;
