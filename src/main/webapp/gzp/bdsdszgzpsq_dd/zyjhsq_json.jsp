...
whereclause="wxxmmc.id=jh.wxxm and jhlx.sxid=jh.jhlx and xb.xldm=jh.xb and  jh.zt='0' and dwid='"+userDwid+"'  order by jhsj desc";
}
...
try {
	...
	String sql_Query_count="select count(*) from z_yxgl_zyjh jh,J_gyjc_xlzd xb,J_GDGY_SXZD jhlx,j_gdgy_wxxm wxxmmc where "+whereclause;
	...
	String sql_Query="select * from (select rownum xh,temp.* from (select jh.*,xb.xlm,jhlx.sxmc,wxxmmc.name from z_yxgl_zyjh jh,J_gyjc_xlzd xb,J_GDGY_SXZD jhlx,j_gdgy_wxxm wxxmmc  where "+ whereclause +") temp) where xh between "+ (index+1) +" and "+ (pageSize + index);
