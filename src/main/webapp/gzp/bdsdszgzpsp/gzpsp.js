Ext.onReady(function() {
	...
	if(dwlb=="1")//判断是网工区
	{
		if(jhlb=="0")//停电作业 绿色
		{
		   ...
		   gzpSp="../ticket_sp/operationTicket_look.jsp?id=";//审批工作票
		 
		}
		else if(jhlb=="1")//带电作业 红色
		{
		
		    gzpSp="../ticket_sp/operationTicket_red_look.jsp?id=";//审批工作票
		
		}
		else//远离作业 黑色
		{
			
		    gzpSp="../ticket_sp/operationTicket_black_look.jsp?id=";//审批工作票
	...
	else//判断是变电所
	{
		if(jhlb=="0")//停电作业 绿色
		{
		  gzpSp="../ticket_sp/gzp_bds_green_look.jsp?id=";//查看工作票
		 
		}
		else if(jhlb=="1")//带电作业 红色
		{
			gzpurl="../ticket_sp/operationTicket_Electric_red.jsp?jhid="+jhid;//添加工作票
		}
		else//远离作业 黑色
		{
			 gzpSp="../ticket/operationTicket_Electric_black.jsp?id=";//查看工作票
		}
	}
	...
	//alert(jhid+"  "+jhlb);
 	...
 	//渲染返回html代码
 	 ...
 	 {gzpSp="../ticket/operationTicket_Electric_black_look.jsp?id=";//查看工作票
    ...
    var store = new Ext.data.Store({
        //proxy告诉我们从哪里获得数据
        proxy: new Ext.data.HttpProxy({url:'gzpsp_json.jsp?jhid='+jhid+'&jhlb='+jhlb+'&dwlb='+dwlb}),      
        //reader告诉我们如何解析这个数据
