Ext.onReady(function() {
    ...
    if(userdj=='6')
    {
	  ...
	  sql="select gqdm,gqjc from j_gyjc_gqzd where jglbdm=2 and gqdm ='"+userdwid+"'";
    }
    else if(userdj=='3')
    {
	  ...
	  sql="select gqdm,gqjc from j_gyjc_gqzd where jglbdm=2 and ddtdm like '"+userdwid+"%25' order by gqjc";
    }
    else if(userdj=='4'||userdj=='5')
    {
	  ...
	  sql="select gqdm,gqjc from j_gyjc_gqzd where jglbdm=2 and cjdm ='"+userdwid+"' order by gqjc";
    }
	else
    {
      stor_where=" 1=1 ";
      stor_where1=" 1=1 ";
	  sql="select gqdm,gqjc from j_gyjc_gqzd where jglbdm=2 order by gqjc";
    }
	...
	var expander = new Ext.ux.grid.RowExpander({
        tpl : new Ext.Template(
            '<p><b>停电范围:</b> {sjgzdd} <b>跳闸原因:</b> {tzyy} </p> <p><b>停电范围车流情况:</b> {gzms} <b>备注:</b> {bz} </p> ')
    ...
    var columns = new Ext.grid.ColumnModel([
		...
		{header:'试送情况',dataIndex:'bztzt',width:60,sortable:true,fixed:true},
    ...
    var store = new Ext.data.Store({
        //proxy告诉我们从哪里获得数据
        proxy: new Ext.data.HttpProxy({url:'tzgl_pd_json.jsp'}),   
