...
try {
	...
	String sql_Query_count="select count(*) from (select yd.*,gq.gqdm as dm,gq.ddtdm from Z_YXGL_QYGZJL yd,j_gyjc_gqzd gq where gq.gqdm=yd.gqpym and yd.ztbz!='0') gz,j_gyjc_gqzd gq where  gz.gqpym=gq.gqdm and "+whereclause;
	...
	String sql_Query="select * from (select rownum xh,temp.* from (select gz.*,to_char(gz.bgsj,'yyyy-mm-dd hh24:mi') bgsj1,to_char(gz.fhsj,'yyyy-mm-dd hh24:mi') fhsj1,to_char(gz.rksj,'yyyy-mm-dd hh24:mi') rksj1,to_char(gz.hfsj,'yyyy-mm-dd hh24:mi') hfsj1,gq.gqmc,gq.gqdm from  (select yd.*,gq.gqdm as dm from Z_YXGL_QYGZJL yd,j_gyjc_gqzd gq where gq.gqdm=yd.gqpym and yd.ztbz!='0') gz,j_gyjc_gqzd gq where  gz.gqpym=gq.gqdm  and "+ whereclause +" order by gz.bgsj desc) temp) where xh between "+ (index+1) +" and "+ (pageSize + index);
    ...
    while(gridResultSet.next()){
		json += "{xh:" +gridResultSet.getLong("xh")  +
				...
				",ia:'"+ convertNull(gridResultSet.getString("ia"))+"'"+//ia
				",ib:'"+ convertNull(gridResultSet.getString("ib"))+"'"+//ib
				",ic:'"+ convertNull(gridResultSet.getString("ic"))+"'"+//ic
				",ua:'"+ convertNull(gridResultSet.getString("ua"))+"'"+//ua
				",ub:'"+ convertNull(gridResultSet.getString("ub"))+"'"+//ub
