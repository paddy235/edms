...
//得到主键
	try 
	{
		sql="select max(id) maxid from Z_JCSJ_gqfbt ";
        ...
        if(isMultiPart){    
         ...
         //初始化返回的文件名数组    
          ...
          if (!fileName.equals("")) {        //到达这个时候，上传文件所必须的条件都满足了，可以上传了，首先要给上传的文件起个名字       
           ...
           writeFile =request.getRealPath("/")+"jcsj\\qxgqfbt\\fbt\\"+ maxid+"."+fileName.substring(fileName.lastIndexOf(".")+1);//文件的全路径        
	   ...
	   //	System.out.println("Name="+Name);
		...
		if(!Name.equals(""))
		{
		sql="insert into Z_JCSJ_gqfbt(id,ZYZDSMC,fbrq,GZZDLJ) values('"+maxid+"','"+ZYZDSMC+"',to_date('"+fbrq+"','yyyy-MM-dd'),'"+Name+"')";
