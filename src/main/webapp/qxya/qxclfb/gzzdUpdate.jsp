        ...
        if(isMultiPart){    
         ...
         //初始化返回的文件名数组    
          ...
          if (!fileName.equals("")) {        //到达这个时候，上传文件所必须的条件都满足了，可以上传了，首先要给上传的文件起个名字       
           ...
           writeFile =request.getRealPath("/")+"jcsj\\qxclfb\\fbt\\"+ xiu_id+"."+fileName.substring(fileName.lastIndexOf(".")+1);//文件的全路径        
	   	...
	   	sql_Upd="update Z_JCSJ_qxclfb set ZYZDSMC='"+ZYZDSMC+"',fbrq=to_date('"+fbrq+"','yyyy-MM-dd'),GZZDLJ='"+Name+"' where ID='"+xiu_id+"'";
	}
	else
	{
    		sql_Upd="update Z_JCSJ_qxclfb set ZYZDSMC='"+ZYZDSMC+"',fbrq=to_date('"+fbrq+"','yyyy-MM-dd') where ID='"+xiu_id+"'";
	}
    if(Name!=""||(num<2&&Name==""))
	{
