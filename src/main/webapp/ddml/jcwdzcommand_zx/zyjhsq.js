   ...
   store_xb.load();
 	...
 	//渲染返回html代码
      	 ...
      	 var zt =function (value)
 	 {
 	 	...
 	 	{
 	 		return '<div  style="color:blue;">待施工</div>';
 	 	}
 	 		else if(value=="3")
 	 	{
 	 		return '施工中';
        ...
        tpl : new Ext.Template(
            '<p><b>停电范围:</b> {nrjyq}</p>',
            '<p><b>周计划编号:</b> {mlh} <b>填报时间:</b> {tbsj} <b>配合单位:</b> {phdw}  <b>要令站:</b> {ylz}</p>',
            '<p><b>线别:</b> {xlm} <b>计划类别:</b> {jhlb} <b>计划类型:</b> {sxmc}  <b>维修项目:</b> {wxxmmc}</p>',
            '<p><b>封锁范围:</b> {fsfw}</p>',
            '<p><b>轨道车运行:</b> {fsfw}</p>',
            '<p><b>备注:</b> {bz}</p>')
    });
    ...
    var columns = new Ext.grid.ColumnModel([
        ...
        {header:'单位名称',dataIndex:'dwmc',width:70,fixed:true,sortable:true},
        //{header:'计划号',dataIndex:'jhh',width:60,sortable:true},
        {header:'作业开始时间',dataIndex:'jhsj',width:120,fixed:true,sortable:true},
        //{header:'线别',dataIndex:'xlm',width:60,fixed:true},
        {header:'行别',dataIndex:'xingb',width:40,fixed:true},
         {header:'施工地点',dataIndex:'sgdd',width:200,fixed:true},
          {header:'作业内容',dataIndex:'jhnr',sortable:true},
       // {header:'计划类别',dataIndex:'jhlb',width:60,sortable:true},
       ...
       // {header:'维修项目',dataIndex:'wxxmmc',width:60,sortable:true},
        //{header:'配合单位',dataIndex:'phdw',width:60,sortable:true},
         {header:'状态',dataIndex:'zt',width:60,renderer:zt,fixed:true}
	...
	var simpleForm_Save = new Ext.FormPanel({
              ...
              buttons : [{
                                   ...
                                   //	var myid = Ext.get('id').value;
                                   	//alert("jhid:"+jhid);
                                   	//alert(jhlb);
                                   	window.location.href='commandsp.jsp?jhid='+jhid;
//                                          
