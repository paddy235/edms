Ext.onReady(function() {
	...
	//定义Grid表头
        ...
        tpl : new Ext.Template(
            '<p><b>操作卡片号:</b> {czkah} <b>停电单元:</b> {tsdqj} <b>操作类型:</b> {czlx} <b>调度员:</b> {ddy}</p>',
            '<p><b>命令内容:</b> {mlnr}</p>',
            '<p><b>备注:</b> {mlbz}</p>')
        });
        ...
        {header:'命令号',dataIndex:'commdid',width:50,fixed:true,sortable:true},
	...
	store_unit.load({params:{start:0, limit:10}});
    
    
    //定义表格面板,指定列模型，仓库，选择模型(默认行选择模型)
    var simple_Grid_unit = new Ext.grid.GridPanel({
                                   ...
                                   handler : function() {
                                          ...
                                          //alert(Ext.getCmp("txt_ksrq").getValue());
                                                    if(Ext.getCmp("txt_ksrq").getValue()!="" ){
														where=where+" and bdsdz.flsj >=to_date('"+Ext.getCmp("txt_ksrq").getValue()+"','yyyy-mm-dd hh24:mi')";	
													    
													}
													 //alert(Ext.getCmp("txt_jsrq").getValue());
													if(Ext.getCmp("txt_jsrq").getValue()!="" ){
														where=where+"  and bdsdz.flsj <=to_date('"+Ext.getCmp("txt_jsrq").getValue()+"','yyyy-mm-dd hh24:mi')";	
													
													}
													///根据计划号进行查询待确认
													if(Ext.getCmp("txt_jhh").getValue()!="" ){
														where=where+"  and bdsdz.jhid =(select id from z_yxgl_zyjh where jhh='"+Ext.getCmp("txt_jhh").getValue()+"')";	
													}
													 ...
													 if(value!=''&& value!='5')
													 {
													    where=where+" and bdsdz.mlzt ="+value;
													}
													...
													{   //alert(Ext.getCmp("dw_id").getValue());
														if(Ext.getCmp("dw_id").getValue()!=''  && Ext.getCmp("dw_id").getValue()!='all'){
															where=where+" and bdsdz.sldwid='"+Ext.getCmp("dw_id").getValue()+"' ";
														
													     }
													}
													else
													{  
													   where=where+" and "+dich_unit1;
													  
													}
												
													//alert(where);
													store_unit.baseParams.whereclause = where; 
				                                    simple_Grid_unit.getStore().reload({ 
														params : { 
														start : 0, 
														limit : 10 
														} 
   ...
   Ext.getCmp("txt_jsrq").setValue(get_nowTime());
     ...
     var simple_Viewport = new Ext.Viewport({
		layout: 'column',
		items: [simpleForm_Query,simple_Grid_unit]
