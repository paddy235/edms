Ext.onReady(function() {
 ...
 dich_swit=dich_swit+" and glkgdz.flsj <=sysdate and glkgdz.flsj>=to_date('"+getTime()+"','yyyy-mm-dd hh24:mi')";
	...
	//定义Grid表头
	  var expander_switch = new Ext.ux.grid.RowExpander({
        tpl : new Ext.Template(
            '<p><b>受令单位:</b> {gqmc} <b>发令时间:</b> {flsj} <b>要求完成时间:</b> {yqwcsj} <b>调度员:</b> {ddy}</p>')
        });
    ...
    //----隔离开关命令网格--------------------------
	//定义Grid表头
    var columns_switch = new Ext.grid.ColumnModel([
        
        //new Ext.grid.RowNumberer(),
        expander_switch,		
        {header:'序号',dataIndex:'xh',width:40,fixed:true},
        {header:'命令号',dataIndex:'commandid',width:50,fixed:true,sortable:true},
        {header:'区间站场',dataIndex:'mc',width:80,sortable:true},
        {header:'隔离开关号',dataIndex:'glkgh',width:50,sortable:true},
        {header:'动作',dataIndex:'dz',width:60,sortable:true},
        {header:'批准时间',dataIndex:'pzsj',width:110,fixed:true,sortable:true},
        {header:'消令时间',dataIndex:'xlsj',width:110,fixed:true},
        {header:'发令人',dataIndex:'flr',width:50,sortable:true},
        {header:'受令人',dataIndex:'slr',width:50,sortable:true},
    ...
    var Record_switch = Ext.data.Record.create([
    	{name:'xh',type:'int'},
    ...
    var store_switch = new Ext.data.Store({
        //proxy告诉我们从哪里获得数据
        proxy: new Ext.data.HttpProxy({url:'command_isolate_switch_json.jsp'}),        
        baseParams:{whereclause:dich_swit},//点击翻页的时候，这个值不会消失，作为基本参数，只有点击查询按钮时才会改变       
	...
	store_switch.load({params:{start:0, limit:10}});
    
    
    //定义表格面板,指定列模型，仓库，选择模型(默认行选择模型)
    var simple_Grid_switch = new Ext.grid.GridPanel({
	...
	var simpleForm_Query = new Ext.FormPanel({
                                   ...
                                   handler : function() {
                                          ...
                                          //alert(Ext.getCmp("txt_ksrq").getValue());
                                                    if(Ext.getCmp("txt_ksrq").getValue()!="" ){
														where_switch=where_switch+" and glkgdz.flsj >=to_date('"+Ext.getCmp("txt_ksrq").getValue()+"','yyyy-mm-dd hh24:mi')";	
													    
													}
													 //alert(Ext.getCmp("txt_jsrq").getValue());
													if(Ext.getCmp("txt_jsrq").getValue()!="" ){
													    where_switch=where_switch+"  and glkgdz.flsj <=to_date('"+Ext.getCmp("txt_jsrq").getValue()+"','yyyy-mm-dd hh24:mi')";	
												 
													}
													///根据计划号进行查询待确认
													if(Ext.getCmp("txt_jhh").getValue()!="" ){
													    where_switch=where_switch+"  and glkgdz.jhid =(select id from z_yxgl_zyjh where jhh='"+Ext.getCmp("txt_jhh").getValue()+"')";	
													 ...
													 if(value!=''&& value!='5')
													 {
													   where_switch=where_switch+" and glkgdz.mlzt ="+value;
													 }
													...
													{   //alert(Ext.getCmp("dw_id").getValue());
														if(Ext.getCmp("dw_id").getValue()!=''  && Ext.getCmp("dw_id").getValue()!='all'){
															where_switch=where_switch+" and glkgdz.sldwid='"+Ext.getCmp("dw_id").getValue()+"' ";
													
													     }
													}
													else
													{
													   where_switch=where_switch+" and "+dich_swit1;
													}
													
													store_switch.baseParams.whereclause = where_switch; 
				                                    simple_Grid_switch.getStore().reload({ 
														params : { 
														start : 0, 
														limit : 10 
														} 
     ...
     var simple_Viewport = new Ext.Viewport({
		layout: 'column',
		items: [simpleForm_Query,simple_Grid_switch]
