Ext.onReady(function() {
    ...
    //alert(bds_mlh);
 	...
 	//渲染返回html代码
      	 ...
      	 var zt =function (value)
 	 {
 	 	...
 	 	{
 	 		return '<div  style="color:red;">执行中</div>';
 	 	}
 	 			else if(value=="4")
 	 	{
   ...
   var sql_gq="select gqdm,gqmc from J_GYJC_GQZD where (jglbdm='1') and ddtdm=\'"+ddtdm+"\'";//
   ...
   var sql_qjzc="select * from J_GYJC_QJZCZD";//
    ...
    //grid.render();//渲染表格
	 ...
	 // form start
      ...
      //  {header:'操作卡片',dataIndex:'GDB',width:60},
        
        {header:'命令内容',dataIndex:'mlnr',width:60},
        {header:'完成时间',dataIndex:'yqwcsj',width:60,sortable:true},
        {header:'批准时间',dataIndex:'pzsj',width:60,sortable:true},
        //{header:'消令时间',dataIndex:'xlsj',width:60,sortable:true},
         {header:'发令人',dataIndex:'flr',width:60,sortable:true},
         {header:'受令人',dataIndex:'slr',width:60,sortable:true},
          {header:'报告人',dataIndex:'zybs',width:60},
           {header:'完成报告单',dataIndex:'tdqd',width:60}
        //{header:'消令人',dataIndex:'xlr',width:60,sortable:true},
    ...
    var store_zy = new Ext.data.Store({
        //proxy告诉我们从哪里获得数据
        proxy: new Ext.data.HttpProxy({url:'commandspzy_json.jsp?jhid='+jhid}),        
        //reader告诉我们如何解析这个数据
        reader: new Ext.data.JsonReader({
            totalProperty: 'totalCount',
            root: 'root',
    ...
    var simple_Grid_zy = new Ext.grid.GridPanel({
                           ...
                           , {
                                   ...
                                   items : [{
                                                        ...
                                                        items : [{
                                                                             ...
                                                                             //allowBlank:fdisabled:true,alse,//不允许为空 
                                                                             blankText:"不能为空，请填写",
                                                                             anchor : '90%'
                                                                            // format:'Y-m-d'
                                                                      }]
              ...
              buttons : [{
                                   ...
                                   handler : function() {
                                     var conn = new Ext.data.Connection();    
									         conn.request({    
									          url : '../../look_card_json.jsp?dwid='+ddtdm+'&LTYPE=3',
									           method: 'GET',    
                                     ...
                                     var jsonObj = Ext.util.JSON.decode(response.responseText);  
                                        //  if( Ext.getCmp("txt_mlh").getValue()==""){	Ext.Msg.alert('消息','命令号不能为空！'); return};
                                                 ...
                                                 simpleFormZy_Save.form.doAction('submit', {
                                                                      ...
                                                                      success : function(form, action) {
                                                                             ...
                                                                             //Ext.Msg.alert('操作',action.result.data);
                                                                      		//Ext.Msg.alert('消息',action.result.msg);
                                                                      		//simpleForm_Query.form.reset();
                                                                      		  Ext.getCmp("txt_mlh").setValue(action.result.msg);
                                                                      		   Ext.getCmp("txt_tdqd").setValue(parseInt(action.result.msg)+100);
     ...
     simpleFormZy_Save.buttons[5].setVisible(true); 
          simpleFormZy_Save.buttons[6].setVisible(false);
    ...
    simple_Grid_zy.addListener('rowclick', function(simple_Grid_zy,rowIndex,event){
    	...
    	//alert(record.get('GDB')+' '+record.get('jhh')+' '+record.get('dwid'));
    	 ...
    	 if(record.get('mlzt')=='2'||record.get('mlzt')=='4')
    	 {
    	simpleFormZy_Save.buttons[2].setVisible(true);
    	simpleFormZy_Save.buttons[3].setVisible(false);
        simpleFormZy_Save.buttons[0].setVisible(true);
