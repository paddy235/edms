...
//System.out.println("TYPEdddddddddddddddddd:"+session.getAttribute("whereclause"));
...
//session.setAttribute("whereclause","");
	...
	if(userdj.equalsIgnoreCase("4")){
		whereclause=" jhlx.sxid=jh.jhlx and xb.xldm=jh.xb and  (jh.zt='0' or jh.zt='1') and dwid like'"+userDwid+"%' order by jhsj desc";
	}else{
		whereclause=" jhlx.sxid=jh.jhlx and xb.xldm=jh.xb and  jh.zt='0' and dwid like'"+userDwid+"%' order by jhsj desc";
	}
}
...
try {
	...
	String sql_Query_count="select count(*) from z_yxgl_zyjh jh,J_gyjc_xlzd xb,J_GDGY_SXZD jhlx where "+whereclause;
	...
	String sql_Query="select * from (select rownum xh,temp.* from (select jh.*,xb.xlm,jhlx.sxmc from z_yxgl_zyjh jh,J_gyjc_xlzd xb,J_GDGY_SXZD jhlx  where "+ whereclause +") temp) where xh between "+ (index+1) +" and "+ (pageSize + index);
