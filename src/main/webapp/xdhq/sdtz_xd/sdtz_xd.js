Ext.onReady(function() {
    ...
    var tsbcolumns = new Ext.grid.ColumnModel([       
        ...
        {header:'供电臂',dataIndex:'SDGDB',width:120,sortable:true},
    ...
    var whereclause_init="ZT=1 and SD_XDT LIKE '"+DWDM+"%'";
    ...
    tsbsimple_Grid.addListener('rowclick', function(tsbsimple_Grid,rowIndex,event){
		...
		Ext.getCmp("id_SD_XDY").setValue(YHMC); 
    ...
    var tsbsimpleForm_Save = new Ext.FormPanel({
              ...
              items : [{
                       ...
                       items : [{xtype:'hidden',name:'SDTZBH',id:'SDTZBH'},{xtype:'hidden',name:'ZT'},
						          {
												columnWidth : .2,
												layout : 'form',
												border : false,
                                            ...
                                            items : [{
														  xtype : 'combo',// 控件的类型设置成combo
														   ...
														   hiddenName : 'SD_XDT',// 大家要注意的是hiddenName和name属性，name只是下拉列表的名称，作用是可通过，而hiddenName才是提交到后台的input的name。如果没有设置hiddenName，在后台是接收不到数据的，这个大家一定要注意。
														   editable : false,// 该下拉列表只允许选择，不允许输入
														   triggerAction : 'all',// 因为这个下拉是只能选择的，所以一定要设置属性triggerAction为all，不然当你选择了某个选项后，你的下拉将只会出现匹配选项值文本的选择项，其它选择项是不会再显示了，这样你就不能更改其它选项了。
														   allowBlank : false,// 该选项值不允许为空
														   fieldLabel : '列调台名',// 控件的标题是学历
														   name : 'SD_XDT',// 再次提醒，name只是下拉列表的名称
														   id:'txt_SD_XDT',                                                               
                                            ...
                                            items : [{
                                                ...
                                                items : [{
														   ...
														   hiddenName : 'SD_DDT',// 大家要注意的是hiddenName和name属性，name只是下拉列表的名称，作用是可通过，而hiddenName才是提交到后台的input的name。如果没有设置hiddenName，在后台是接收不到数据的，这个大家一定要注意。
														   editable : false,// 该下拉列表只允许选择，不允许输入
														   triggerAction : 'all',// 因为这个下拉是只能选择的，所以一定要设置属性triggerAction为all，不然当你选择了某个选项后，你的下拉将只会出现匹配选项值文本的选择项，其它选择项是不会再显示了，这样你就不能更改其它选项了。
														   allowBlank : false,// 该选项值不允许为空
														   fieldLabel : '电调台名',// 控件的标题是学历
														   name : 'SD_DDT',// 再次提醒，name只是下拉列表的名称
														   id:'txt_SD_DDT',                                                               
														   anchor : '96%'// input的宽度是90%
                                                        }]
                                            }]
