...
<%!   
    ...
    //查询计划对应的馈线
    DbTrade tdqd_connection=new DbTrade();
    public String AddTdqd(String jhh)
    {
       String strTdqd="";
       String tdqd_sql="select tdqdmc from z_yxgl_zyjh_Tdqd where jhid='"+jhh+"' ";
       try
       {
         java.sql.ResultSet tdqd =tdqd_connection.executeQuery(tdqd_sql);
         while(tdqd.next())
         {
           strTdqd=strTdqd+tdqd.getString("tdqdmc")+"、";
         }
...
try {
   ...
   //	System.out.println("index:"+index);
	...
	String sql_Query_count="select count(*) from z_yxgl_zyjh jh,J_gyjc_xlzd xb,J_GDGY_SXZD jhlx,J_GYJC_GQZD gq,z_yxgl_zyjh_Tdqd tdqd where jh.id=tdqd.jhid(+) and gq.gqdm=jh.dwid and jhlx.sxid=jh.jhlx and xb.xldm=jh.xb and "+whereclause;
	...
	String sql_Query="select * from (select rownum xh,temp.* from (select jh.*,xb.xlm,jhlx.sxmc,tdqd.tdqdmc from z_yxgl_zyjh jh,J_gyjc_xlzd xb,J_GDGY_SXZD jhlx, J_GYJC_GQZD gq,z_yxgl_zyjh_Tdqd tdqd  where jh.id=tdqd.jhid(+) and  gq.gqdm=jh.dwid  and jhlx.sxid=jh.jhlx and xb.xldm=jh.xb and "+ whereclause +" order by jh.jhsj desc) temp) where xh between "+ (index+1) +" and "+ (pageSize + index);
