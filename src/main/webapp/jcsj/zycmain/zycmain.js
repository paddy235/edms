    ...
    */
    //渲染返回html代码
 	 ...
 	 //定义Grid表头
        ...
        {header:'车号',dataIndex:'ch',sortable:true},
        {header:'所属车间',dataIndex:'cjmc',sortable:true},
        {header:'出厂日期',dataIndex:'ccrq',sortable:true},
        {header:'设备编号',dataIndex:'sbbh',sortable:true},
        {header:'车型',dataIndex:'cx',sortable:true},
        //{header:'载重',dataIndex:'zz',sortable:true},
        {header:'停放地点',dataIndex:'gqmc',sortable:true},
        {header:'公里标',dataIndex:'glb',sortable:true},
        {header:'发动机功率',dataIndex:'fdjgl',sortable:true},
        //{header:'通过最小曲线半径（米）',dataIndex:'tgzxqxbj',width:60,fixed:true},
        //{header:'最大运行速度（km/h）',dataIndex:'zzyxsd',fixed:true},
        ...
        items : [{
                	...
                	items:[{xtype:'hidden',name:'id'},{xtype:'hidden',name:'albm'},{
                		...
                		items: [{
                        	...
                        	mode : 'local',// 数据是在本地
                        	//forceSelection : true,// 必须选择一个选项
                        	blankText : '请选所在车间',// 提交form时，该项如果没有选择，则提示错误信息"请选择学历"
                        	emptyText : '选择车间',// 在没有选择值时显示为选择学历
                        	hiddenName : 'sscj',// 大家要注意的是hiddenName和name属性，name只是下拉列表的名称，作用是可通过，而hiddenName才是提交到后台的input的name。如果没有设置hiddenName，在后台是接收不到数据的，这个大家一定要注意。
                        	editable : false,// 该下拉列表只允许选择，不允许输入
                        	triggerAction : 'all',// 因为这个下拉是只能选择的，所以一定要设置属性triggerAction为all，不然当你选择了某个选项后，你的下拉将只会出现匹配选项值文本的选择项，其它选择项是不会再显示了，这样你就不能更改其它选项了。
                        	allowBlank : false,// 该选项值不允许为空
                        	fieldLabel : '所属车间',// 控件的标题是学历
                        	name : 'sscj',// 再次提醒，name只是下拉列表的名称
