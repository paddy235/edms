...
try {
	...
	String sql_Query_count="select count(*) from z_jcsj_xianlu where "+whereclause;

	java.sql.ResultSet gridResultSet_count=db_connection.executeQuery(sql_Query_count);
	gridResultSet_count.next();
	totalCount=gridResultSet_count.getInt(1);
	
	String sql_Query="select * from (select rownum xh,temp.* from (select * from z_jcsj_xianlu where "+ whereclause +" order by ID desc) temp) where xh between "+ (index+1) +" and "+ (pageSize + index);
...
while(gridResultSet.next()){
		json += "{ID:" +gridResultSet.getInt("ID")  + 
        		",XLMC:'"+gridResultSet.getString("XLMC") +"'" +
        		...
        		",CD:'"+gridResultSet.getString("CD")+"'"+
        		",ZL:'"+gridResultSet.getString("ZL")+"'"+
