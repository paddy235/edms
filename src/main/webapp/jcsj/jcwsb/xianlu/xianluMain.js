Ext.onReady(function(){
    ...
    */
    //渲染返回html代码
 	 ...
 	 {
 	     return '<a  href="xianluDel.jsp?ID='+value+'">删除</a>';
 	 };
 	  var expander = new Ext.ux.grid.RowExpander({
        tpl : new Ext.Template(
            '<p><b>张力:</b> {ZL}    <b>生产日期:</b> {SCRQ}<b>投运日期:</b> {TYRQ}  <b>生产厂家:</b> {SCCJ}</p>' 	)
    });	
 	 //定义Grid表头
 	 // (ID,'XLMC','QJZC','XBBM','SXXBM','SBXH','SBCZ','CD','BZ','ZL','SCRQ','TYRQ','SCCJ')
        ...
        {header:'线路名称',dataIndex:'XLMC',sortable:true,width:60,sortable:true},
        ...
        {header:'设备型号',dataIndex:'SBXH',sortable:true,width:60,sortable:true},                
       	{header:'设备材质',dataIndex:'SBCZ',sortable:true,width:60,sortable:true},
       	{header:'长度',dataIndex:'CD',sortable:true,width:60,sortable:true},
       	//{header:'张力',dataIndex:'ZL',sortable:true,width:60,sortable:true},
	 ...
	 var store = new Ext.data.Store({
     	//proxy告诉我们从哪里获得数据
     	proxy: new Ext.data.HttpProxy({url:'xianlulist.jsp'}),        
     	//reader告诉我们如何解析这个数据
              ...
              items: [{
                		...
                		items: [{
                        	...
                        	store : new Ext.data.SimpleStore({
                        		...
                        		data : [['', '全部'],['接触线', '接触线'],['承力索', '承力索'], ['供电线', '供电线'],['回流线', '回流线'],['加强线', '加强线'], ['架空地线', '架空地线']]
                        	}),
                        	valueField : "returnValue",// 设置下拉选择框的值
                       		displayField : "displayText",// 设置下拉选择框的显示文本
                        	mode : 'local',// 数据是在本地
                        	//forceSelection : true,// 必须选择一个选项
                        	blankText : '请选择线路名称',// 提交form时，该项如果没有选择，则提示错误信息"请选择学历"
                        	emptyText : '选择线路名称',// 在没有选择值时显示为选择学历
                        	hiddenName : 'XLMC',// 大家要注意的是hiddenName和name属性，name只是下拉列表的名称，作用是可通过，而hiddenName才是提交到后台的input的name。如果没有设置hiddenName，在后台是接收不到数据的，这个大家一定要注意。
                        	editable : false,// 该下拉列表只允许选择，不允许输入
                        	triggerAction : 'all',// 因为这个下拉是只能选择的，所以一定要设置属性triggerAction为all，不然当你选择了某个选项后，你的下拉将只会出现匹配选项值文本的选择项，其它选择项是不会再显示了，这样你就不能更改其它选项了。
                        	allowBlank : false,// 该选项值不允许为空
                        	fieldLabel : '线路名称',// 控件的标题是学历
                        	name : 'XLMC',// 再次提醒，name只是下拉列表的名称
                	...
                	items:[{
                		...
                		items: [{
                        	...
                        	store : new Ext.data.SimpleStore({
                        		...
                        		fields : ["returnValue", "displayText"],
                        		// 定义了几组数据.每组数据都是根据fiedls的定义来组成的，数组里第一个值就是retrunValue的值，第二个值就是displayText的值，例如[1,'小学']，就表示retrunValue是1，displayText是小学。
                        		data : [['接触线', '接触线'],['承力索', '承力索'], ['供电线', '供电线'],['回流线', '回流线'],['加强线', '加强线'], ['架空地线', '架空地线']]
                        	}),
                        	valueField : "returnValue",// 设置下拉选择框的值
                       		displayField : "displayText",// 设置下拉选择框的显示文本
                        	mode : 'local',// 数据是在本地
                	...
                	items:[{
                		...
                		items: [{
                        	...
                        	store : new Ext.data.SimpleStore({
                        		...
                        		fields : ["returnValue", "displayText"],
                        		// 定义了几组数据.每组数据都是根据fiedls的定义来组成的，数组里第一个值就是retrunValue的值，第二个值就是displayText的值，例如[1,'小学']，就表示retrunValue是1，displayText是小学。
                        		data : [['CuAgRis100', 'CuAgRis100'],['CuMg', 'CuMg'], ['lj240', 'lj240'],['lj235', 'lj235']]
                        	}),
                        	valueField : "returnValue",// 设置下拉选择框的值
                       		displayField : "displayText",// 设置下拉选择框的显示文本
                        	mode : 'local',// 数据是在本地
                        	//forceSelection : true,// 必须选择一个选项
                        	blankText : '请选择设备型号',// 提交form时，该项如果没有选择，则提示错误信息"请选择学历"
                        	emptyText : '选择设备型号',// 在没有选择值时显示为选择学历
                        	hiddenName : 'SBXH',// 大家要注意的是hiddenName和name属性，name只是下拉列表的名称，作用是可通过，而hiddenName才是提交到后台的input的name。如果没有设置hiddenName，在后台是接收不到数据的，这个大家一定要注意。
                        	editable : false,// 该下拉列表只允许选择，不允许输入
                        	triggerAction : 'all',// 因为这个下拉是只能选择的，所以一定要设置属性triggerAction为all，不然当你选择了某个选项后，你的下拉将只会出现匹配选项值文本的选择项，其它选择项是不会再显示了，这样你就不能更改其它选项了。
                        	allowBlank : false,// 该选项值不允许为空
                        	fieldLabel : '设备型号',// 控件的标题是学历
                        	name : 'SBXH',// 再次提醒，name只是下拉列表的名称
                        	anchor : '95%'// input的宽度是90%
    					}]	
                	},{
                		columnWidth:.32,
                		...
                		items: [{
                        	...
                        	store : new Ext.data.SimpleStore({
                        		...
                        		fields : ["returnValue", "displayText"],
                        		// 定义了几组数据.每组数据都是根据fiedls的定义来组成的，数组里第一个值就是retrunValue的值，第二个值就是displayText的值，例如[1,'小学']，就表示retrunValue是1，displayText是小学。
                        		data : [['铜/合金', '铜/合金'],['铝绞线', '铝绞线']]
                        	}),
                        	valueField : "returnValue",// 设置下拉选择框的值
                       		displayField : "displayText",// 设置下拉选择框的显示文本
                        	mode : 'local',// 数据是在本地
                        	//forceSelection : true,// 必须选择一个选项
                        	blankText : '请选择设备材质',// 提交form时，该项如果没有选择，则提示错误信息"请选择学历"
                        	emptyText : '选择设备材质',// 在没有选择值时显示为选择学历
