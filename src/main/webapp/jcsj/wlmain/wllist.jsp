...
try {
	...
	String sql_Query_count="select count(*) from Z_jcsj_wuliao where "+whereclause;
	//System.out.println("session:"+sql_Query_count);
	java.sql.ResultSet gridResultSet_count=db_connection.executeQuery(sql_Query_count);
	gridResultSet_count.next();
	totalCount=gridResultSet_count.getInt(1);
	
	String sql_Query="select * from (select rownum xh,temp.* from (select a.* ,b.gqmc,c.sbflname from z_jcsj_wuliao a,j_gyjc_gqzd b,j_gdgy_sbfl c where "+ whereclause +" and a.mc=c.sbflid and a.ssdw=b.gqdm order by ID desc) temp) where xh between "+ (index+1) +" and "+ (pageSize + index);
    ...
    while(gridResultSet.next()){
		json += "{id:" +gridResultSet.getInt("id")  + 
        		",ccxh:"+gridResultSet.getInt("ccxh") +
