    ...
    */
    //渲染返回html代码
 	 ...
 	 //定义Grid表头
        ...
        {header:'桥梁名称',dataIndex:'sxgh',sortable:true},
        {header:'单位名称',dataIndex:'gqmc',sortable:true},
        {header:'区段',dataIndex:'qjzcmc',sortable:true},
        
        {header:'公里标',dataIndex:'glb',sortable:true},        
        {header:'两侧支柱',dataIndex:'xxgh',sortable:true},
        //{header:'护栏结构形式高度',dataIndex:'hljgxs',sortable:true},
        //{header:'跨桥长度',dataIndex:'kqcd',sortable:true},
        //{header:'护栏尺寸',dataIndex:'hlcc',sortable:true},
        //{header:'距外轨尺寸',dataIndex:'jwgcc',width:60,fixed:true},
        {header:'产权所属单位',dataIndex:'cqssdw',fixed:true},
        {header:'联系电话',dataIndex:'lxdh',fixed:true},
     ...
     var planRecord = Ext.data.Record.create([
    	...
    	{name:'sxgh',type:'string'},
        {name:'xxgh',type:'string'},
        {name:'hljgxs',type:'string'},
    	{name:'kqcd',type:'string'},
    	{name:'hlcc',type:'string'},
    	{name:'jwgcc',type:'string'},
    	{name:'cqssdw',type:'string'},
    	{name:'lxdh',type:'string'},
       	{name:'gqmc',type:'string'},
    	{name:'xlmc',type:'string'},
    	{name:'qjzcmc',type:'string'},{name:'bz',type:'string'}
     ]);
	 ...
	 var store = new Ext.data.Store({
     	//proxy告诉我们从哪里获得数据
     	proxy: new Ext.data.HttpProxy({url:'skqlist.jsp'}),        
     	//reader告诉我们如何解析这个数据
        ...
        items : [{
                	...
                	items:[{
                		...
                		items: [{
                        	...
                        	mode : 'local',// 数据是在本地
                        	//forceSelection : true,// 必须选择一个选项
                        	blankText : '请选择区段',// 提交form时，该项如果没有选择，则提示错误信息"请选择学历"
                        	emptyText : '选择区段',// 在没有选择值时显示为选择学历
                        	hiddenName : 'qd',// 大家要注意的是hiddenName和name属性，name只是下拉列表的名称，作用是可通过，而hiddenName才是提交到后台的input的name。如果没有设置hiddenName，在后台是接收不到数据的，这个大家一定要注意。
                        	editable : false,// 该下拉列表只允许选择，不允许输入
                        	triggerAction : 'all',// 因为这个下拉是只能选择的，所以一定要设置属性triggerAction为all，不然当你选择了某个选项后，你的下拉将只会出现匹配选项值文本的选择项，其它选择项是不会再显示了，这样你就不能更改其它选项了。
                        	allowBlank : false,// 该选项值不允许为空
                        	fieldLabel : '区段',// 控件的标题是学历
                        	name : 'qd',// 再次提醒，name只是下拉列表的名称
                        	id:'txt_qjzc',                        	
                        	anchor : '95%'// input的宽度是90%
    					}]	
                	},{
