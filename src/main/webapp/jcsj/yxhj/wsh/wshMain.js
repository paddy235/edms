    ...
    */
    //渲染返回html代码
 	 ...
 	 //定义Grid表头
        ...
        {header:'供电线路名称',dataIndex:'gdxlmc',sortable:true},
        {header:'区站站场',dataIndex:'qjzcmc',sortable:true},
        {header:'公里标',dataIndex:'glb',sortable:true},
        {header:'支柱号',dataIndex:'jcwgh',sortable:true},
        {header:'高度',dataIndex:'gd',sortable:true},
        {header:'树种',dataIndex:'sz',sortable:true},
        //{header:'修枝数量',dataIndex:'szsl',sortable:true},
        //{header:'截干数量',dataIndex:'jgsl',sortable:true},
        //{header:'砍伐数量',dataIndex:'kfsl',sortable:true},
        //{header:'是否路产',dataIndex:'sflc',sortable:true},
        //{header:'是否铁路地界内',dataIndex:'sftldjn',sortable:true},
        {header:'工区联系人',dataIndex:'gqlxr',sortable:true},
        {header:'工区联系电话',dataIndex:'lxdh',sortable:true},
        //{header:'产权所属单位',dataIndex:'cqdw',fixed:true},
        //{header:'产权单位联系人',dataIndex:'cqdwlxr',sortable:true},
        //{header:'产权单位联系电话',dataIndex:'cqdwlxrdh',fixed:true},
        //{header:'补偿金额',dataIndex:'bcje',sortable:true},
        //{header:'砍伐日期',dataIndex:'kfrq',sortable:true}, 
     ...
     var planRecord = Ext.data.Record.create([
        ...
        {name:'gdxlmc',type:'string'}, 
        {name:'qjzc',type:'string'},
    	{name:'glb',type:'string'},
    	{name:'jcwgh',type:'string'},
        {name:'gd',type:'string'},
        {name:'sz',type:'string'},
    	{name:'szsl',type:'string'},
    	{name:'jgsl',type:'string'},
    	{name:'kfsl',type:'string'},
    	{name:'sflc',type:'string'},
    	{name:'sftldjn',type:'string'},
    	{name:'gqlxr',type:'string'},
        {name:'lxdh',type:'string'},
    	{name:'cqdw',type:'string'},
    	{name:'cqdwlxr',type:'string'},
    	{name:'cqdwlxrdh',type:'string'},
    	{name:'bcje',type:'string'},
    	{name:'kfrq',type:'string'},
	 ...
	 var store = new Ext.data.Store({
     	//proxy告诉我们从哪里获得数据
     	proxy: new Ext.data.HttpProxy({url:'wshlist.jsp'}),        
     	//reader告诉我们如何解析这个数据
     	reader: new Ext.data.JsonReader({
     		totalProperty: 'totalCount',
     		root: 'root',
                	...
                	items:[{
                		...
                		items: [{
                        	...
                        	store : new Ext.data.SimpleStore({
                        		...
                        		data : [['是', '是'],['否', '否'],['不确定', '不确定']]
                        	}),
                        	valueField : "returnValue",// 设置下拉选择框的值
                       		displayField : "displayText",// 设置下拉选择框的显示文本
                        	mode : 'local',// 数据是在本地
                        	//forceSelection : true,// 必须选择一个选项
                        	blankText : '请选择是否路产',// 提交form时，该项如果没有选择，则提示错误信息"请选择学历"
                        	emptyText : '选择是否路产',// 在没有选择值时显示为选择学历
                        	hiddenName : 'sflc',// 大家要注意的是hiddenName和name属性，name只是下拉列表的名称，作用是可通过，而hiddenName才是提交到后台的input的name。如果没有设置hiddenName，在后台是接收不到数据的，这个大家一定要注意。
                        	editable : false,// 该下拉列表只允许选择，不允许输入
                        	triggerAction : 'all',// 因为这个下拉是只能选择的，所以一定要设置属性triggerAction为all，不然当你选择了某个选项后，你的下拉将只会出现匹配选项值文本的选择项，其它选择项是不会再显示了，这样你就不能更改其它选项了。
                        	allowBlank : false,// 该选项值不允许为空
                        	fieldLabel : '是否路产',// 控件的标题是学历
                        	name : 'sflc',// 再次提醒，name只是下拉列表的名称
                        	anchor : '95%'// input的宽度是90%
    					}]	
                	},{
                		columnWidth:.32,
                		...
                		items: [{
                        	...
                        	mode : 'local',// 数据是在本地
                        	//forceSelection : true,// 必须选择一个选项
                        	blankText : '请选择是否铁路地界内',// 提交form时，该项如果没有选择，则提示错误信息"请选择学历"
                        	emptyText : '选择是否铁路地界内',// 在没有选择值时显示为选择学历
                        	hiddenName : 'sftldjn',// 大家要注意的是hiddenName和name属性，name只是下拉列表的名称，作用是可通过，而hiddenName才是提交到后台的input的name。如果没有设置hiddenName，在后台是接收不到数据的，这个大家一定要注意。
                        	editable : false,// 该下拉列表只允许选择，不允许输入
                        	triggerAction : 'all',// 因为这个下拉是只能选择的，所以一定要设置属性triggerAction为all，不然当你选择了某个选项后，你的下拉将只会出现匹配选项值文本的选择项，其它选择项是不会再显示了，这样你就不能更改其它选项了。
                        	allowBlank : false,// 该选项值不允许为空
                        	fieldLabel : '是否铁路地界内',// 控件的标题是学历
                        	name : 'sftldjn',// 再次提醒，name只是下拉列表的名称
                        	anchor : '95%'// input的宽度是90%
    					}]	
                	},{
                	    columnWidth:.32,
