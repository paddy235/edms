...
try {
	...
	String sql_Query_count="select count(*) from Z_JCSJ_SDXJ where "+whereclause;
	//System.out.println("session:"+sql_Query_count);
	java.sql.ResultSet gridResultSet_count=db_connection.executeQuery(sql_Query_count);
	gridResultSet_count.next();
	totalCount=gridResultSet_count.getInt(1);
	
	String sql_Query="select * from (select rownum xh,temp.* from (select a.*,b.gqmc,c.xlm,d.mc from  Z_JCSJ_SDXJ a,j_gyjc_gqzd b,j_gyjc_xlzd c ,j_gyjc_qjzczd d where "+ whereclause +" and a.xzjg=b.gqdm and a.xb=c.xldm and a.qjzc=d.qjbm order by ID desc) temp) where xh between "+ (index+1) +" and "+ (pageSize + index);
    ...
    while(gridResultSet.next()){
		json += "{id:" +gridResultSet.getInt("id")  + 
        		...
        		",sdh:'"+CheckSting(gridResultSet.getString("sdh"))+"'"+
        		",sdm:'"+CheckSting(gridResultSet.getString("sdm"))+"'"+
        		",sxx:'"+CheckSting(gridResultSet.getString("sxx"))+"'"+
        		",jcxzdgd:'"+CheckSting(gridResultSet.getString("jcxzdgd"))+"'"+        		
        		",sdcd:'"+CheckSting(gridResultSet.getString("sdcd"))+"'"+
