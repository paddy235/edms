...
<%
    ...
    try {
        ...
        String sql_Query_count = "select count(*) from Z_jcsj_byq where " + whereclause;
        //System.out.println("session:"+sql_Query_count);
        java.sql.ResultSet gridResultSet_count = db_connection.executeQuery(sql_Query_count);
        gridResultSet_count.next();
        totalCount = gridResultSet_count.getInt(1);

        String sql_Query = "select * from (select rownum xh,temp.* from (select * from Z_JCSJ_byq where " + whereclause + " order by ID desc) temp) where xh between " + (index + 1) + " and " + (pageSize + index);
        ...
        while (gridResultSet.next()) {
            json += "{id:" + gridResultSet.getInt("id") +
                    ...
                    ",csrq:'" + (gridResultSet.getDate("csrq") == null ? "" : (frmDate.format(gridResultSet.getDate("csrq")).toString())) + "'" +
